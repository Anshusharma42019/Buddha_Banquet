...
const UpdateBooking = () => {
  ...
  const fetchBookingDetail = async () => {
    try {
      // Fetch booking data
      const bookingResponse = await axios.get(`http://localhost:4000/api/bookings/get/${id}`);
      
      // Fetch associated menu data
      let categorizedMenu = null;
      try {
        const menuResponse = await axios.get(`http://localhost:4000/api/menus/${id}`);
  ...
  const updateBooking = () => {
    ...
    if (role !== "Admin") {
      ...
      axios
        .get(`http://localhost:4000/api/bookings/get/${id}`)
        .then((res) => {
  ...
  const performUpdate = (staffEditCount) => {
    ...
    axios
      .put(`http://localhost:4000/api/bookings/update/${id}`, payload)
      .then((res) => {
        if (res.data) {
